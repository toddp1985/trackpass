# TrackPass Instagram Launch Queue — @trackpassgolf
# 7 posts to publish before any outreach. Post 2x/day. Use Canva for all graphics.
# Account: Branded — @trackpassgolf, no connection to any personal account.
# Setup: sign up with hello@trackpassgolf.com. No linked personal phone or Facebook.

---

## POST 1 — Hero Intro (Day 1, 9am)
**Format:** Single image or short Reel (15s)
**Visual:** Texas state outline filled with golf course green, headline text overlay
**Canva:** Start from "Instagram Post" > "Minimalist" template > dark green background, white text

**Caption:**
Every public golf course in Texas.
One pass. $199/year.

2 free rounds/yr at partner courses. Green fee reimbursed at any other TX public course.
No blackout dates. No reservations required.

trackpassgolf.com — link in bio.

#TexasGolf #GolfTexas #MunicipalGolf #PublicGolf #GolfPass #TexasGolfer

---

## POST 2 — Course Spotlight: TPC Craig Ranch (Day 1, 6pm)
**Format:** Carousel (5 slides)
**Visual:** Course photos + stats
**Canva:** "Instagram Carousel" > "Travel" template > green palette

**Slide 1:** "Course Spotlight: TPC Craig Ranch, McKinney TX"
**Slide 2:** Photo of course + "Par 72 | 7,437 yards | Rated one of Texas's best public courses"
**Slide 3:** "Walk-up rate: $85-$150/round"
**Slide 4:** "TrackPass members: green fee reimbursed up to $50/round. (Partner courses get 2 free rounds/yr.)"
**Slide 5:** "Your $199 pass covers this and 99 other TX public courses. trackpassgolf.com"

**Caption:**
TPC Craig Ranch. McKinney, Texas.

One of the finest public courses in the state — pay the green fee here and TrackPass reimburses you up to $50.

A single round runs $85–$150. Your $199 pass covers reimbursement here and at 99 other TX public courses. Partner courses unlock 2 free rounds/yr when they join the network.

trackpassgolf.com

#TPCCraigRanch #McKinneyGolf #TexasGolf #GolfPass #DFWGolf

---

## POST 3 — Savings Math (Day 2, 9am)
**Format:** Single graphic
**Visual:** Clean white card, large numbers
**Canva:** "Instagram Post" > "Bold Typography" > white bg, dark green text

**Graphic text:**
4 rounds at Texas munis = $140–$200
TrackPass annual membership = $199

Play 4 rounds. The pass pays for itself.
Partner-course rounds are free; everywhere else, your green fee gets reimbursed.

**Caption:**
The math is simple.

The average Texas public course charges $40–60/round. Play 4 times and your TrackPass has paid for itself.

100 TX public courses. $199/year.

trackpassgolf.com

#GolfPass #TexasGolf #GolfDeals #MunicipalGolf #AffordableGolf

---

## POST 4 — Course Spotlight: Firewheel Golf Park (Day 2, 6pm)
**Format:** Carousel (5 slides)
**Visual:** Firewheel photos (Wikimedia has one) + stats

**Caption:**
Firewheel Golf Park. Garland, Texas.

36 holes of championship public golf in DFW — pay the green fee and TrackPass reimburses you up to $50.

Walk-up: $45–$65/round. Your $199 pass covers reimbursement here + 99 other TX courses.

trackpassgolf.com

#FirewheelGolf #GarlandGolf #DFWGolf #TexasGolf #GolfPass

---

## POST 5 — How It Works (Day 3, 9am)
**Format:** Carousel (4 slides)
**Visual:** Step-by-step numbered cards
**Canva:** "Instagram Carousel" > numbered list style > green accent

**Slide 1:** "How TrackPass Works"
**Slide 2:** "1. Buy your pass ($199/yr). Get a digital member card."
**Slide 3:** "2. At partner courses: show your card at the pro shop. Walk on."
**Slide 4:** "3. Anywhere else: pay the green fee, log your round, submit the receipt — reimbursed up to $50/round, 1/month, up to $199/yr."

**Caption:**
No apps to book. No blackout dates.

At partner courses, show your TrackPass card at the pro shop and walk on. Anywhere else, pay the green fee and get reimbursed.

$199/year. 100 TX public courses.

trackpassgolf.com — link in bio.

#GolfPass #TexasGolf #SimpleGolf #MunicipalGolf

---

## POST 6 — Course Spotlight: Hermann Park (Day 3, 6pm)
**Format:** Carousel (5 slides)
**Visual:** Hermann Park golf clubhouse photo (Wikimedia: commons/3/38/Hermann_Park_Municipal_Golf_Clubhouse...)

**Caption:**
Hermann Park Golf Course. Houston, Texas.

Steps from the Museum of Natural Science and the Houston Zoo — one of Houston's most beloved public courses.

TrackPass members: pay the green fee here and get reimbursed up to $50/round. Visitors pay $30–$45.

trackpassgolf.com

#HermannPark #HoustonGolf #TexasGolf #GolfPass #PublicGolf

---

## POST 7 — Founding Member CTA (Day 4, 9am)
**Format:** Single graphic
**Visual:** Bold countdown feel, limited offer framing
**Canva:** "Instagram Post" > "Announcement" template > dark green bg, white/gold text

**Graphic text:**
Founding Member Pricing
$199/year
(Price will increase as courses join the network)

**Caption:**
We're building the Texas golf network.

Right now, TrackPass founding members get access to 100 TX public courses for $199/year. As more courses join as official partners, that price goes up.

Lock in founding member pricing now.

trackpassgolf.com — link in bio.

#TexasGolf #GolfPass #FoundingMember #GolfDeals #TexasGolfer

---

## Bio Copy
**Name field:** TrackPass
**Username:** @trackpassgolf
**Bio:**
Texas's golf pass 🏌️
$199/yr · 100 TX public courses
2 rounds/yr at partner courses
↓ Lock in founding pricing

**Link:** https://trackpassgolf.com?utm_source=instagram

---

## Account Setup (Anonymous)
1. Email: hello@trackpassgolf.com — sign up with this (domain email, zero personal name connection)
2. Sign up at instagram.com with hello@trackpassgolf.com
3. No phone — skip or use a Google Voice number not linked to personal accounts
4. Profile photo: Texas flag + golf pin icon (generate in Canva)
5. No location set on account profile
6. UTM on every bio link: ?utm_source=instagram

## Posting Schedule
- 9am CT daily (course spotlights, savings math, how-it-works)
- 6pm CT daily (course spotlights, engagement posts)
- Use Meta Business Suite (business.facebook.com) — FREE, built by Instagram/Meta, schedule directly
  - Connect @trackpassgolf to a new Facebook Page (TrackPass Golf) during signup
  - No third-party needed, no cost, no post limits
- Respond to every comment within 2 hours for the first 30 days
